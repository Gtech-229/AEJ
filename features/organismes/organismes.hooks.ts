...
import type { CreateOrganismePayload, UpdateOrganismePayload } from './organismes.dto';
...
export function useUpdateOrganisme() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (payload: UpdateOrganismePayload) => organismesService.update(payload),
    onSuccess: () => {
